...
def make_generator_model():
  ...
  # model.add(layers.Dense(256*256*3, use_bias = False, input_shape = (256, 256, 3)))
  ...
  model.add(layers.Conv2DTranspose(16, (5, 5), strides=(16, 16), padding='same', use_bias=False))
  print(model.output_shape)
...
@tf.function
def train_step(target, inputimg, step):
  noise = tf.random.normal([BATCH_SIZE, 100])

  with tf.GradientTape() as gen_tape, tf.GradientTape() as disc_tape:
    generated_images = generator(inputimg, training=True)
...
def fit(train_ds, test_ds, steps):
  ...
  for step, (input_image, target) in train_ds.repeat().take(steps).enumerate():
    ...
    train_step(target, input_image, step)
